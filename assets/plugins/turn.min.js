...
turnMethods = {
	...
	init: function(opt) {
		...
		for (p = 1; p <= l; p++) {
			...
			d.pageWrap[p] = $('<div/>', {'class': 'turn-page-wrapper', css: {position: 'absolute', overflow: 'hidden'}}).
					attr('page', p).
						appendTo(this).
	...
	_end: function(e, turned) {
		...
		var that = $(this),
			...
			if (turned || dd.tpage) {
				...
				if (dd.tpage==o.next || dd.tpage==o.page) { 
			
					dd.page = dd.tpage || o.next;
					delete dd['tpage'];


					turn.trigger('end', [dd.page]);
	...
	update: function() {
		...
		if (d.pageMv.length) {
        	...
        	for (p = 1; p <= d.totalPages; p++) { 
        		...
        		if (d.tpage)
        			d.pages[p].flip('disable', d.disabled || p!=apage);
        	}
			...
			for (p = 1; p <= d.totalPages; p++) {
				...
				d.pages[p].flip('disable', d.disabled || !isFront);
			}
...
flipMethods = {
	...
	_cornerActivated: function(e) {
		...
		return ($.inArray(corner, cAllowed)==-1) ? false : {corner: corner, x: x, y: y};
	...
	_showThumbIndex: function(c, interpolate) {
		...
		if (d.opt.back) {
			...
			if (interpolate) {
				...
				var that = this, p = (d.p && d.p.corner==c.corner) ? d.p : flipMethods._c.call(this, c.corner, 1);
	...
	_addEvents: function() {
		...
		var that = this,
			...
			$(d.opt.turn || document).bind(events.start, function(e) {
				return flipMethods._eventStart.call(that, e); 
			});

			$(document).bind(events.move, function(e) {
				flipMethods._eventMove.call(that, e);
			}).
			bind(events.end, function(e) {
				flipMethods._eventEnd.call(that, e);
			});
	...
	_eventMove: function(e) {
		...
		if (!d.disabled) {
			var e = (isTouch) ? e.originalEvent.touches : [e];
